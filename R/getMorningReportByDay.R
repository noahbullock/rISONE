...
#' @export
getMorningReportByDay <- function(day = Sys.Date(), user = getOption(x = "ISO_NE_USER"), password = getOption(x = "ISO_NE_PASSWORD"), 
                                           out.tz = "America/New_York", ...){
  dd <- format(as.Date(day), "%Y%m%d")
  
  json <- get_path(path = paste0("/morningreport/day/", dd), user = user, password = password, ...)
